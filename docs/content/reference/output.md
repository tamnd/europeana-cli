...
describes what users get. Pick a format with `-o`, or let europeana choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
europeana <command> -o table   # aligned columns for reading
europeana <command> -o jsonl   # one JSON object per line, for piping
europeana <command> -o json    # a single JSON array
europeana <command> -o csv     # spreadsheet friendly
europeana <command> -o tsv     # tab-separated
europeana <command> -o url     # just the URL column
europeana <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
europeana <command> --fields id,title,url
```
...
```bash
europeana <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
europeana <command>            # a table, because this is a terminal
europeana <command> | wc -l    # JSONL, because this is a pipe
```
