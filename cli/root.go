// Package cli assembles the europeana command tree from the europeana
...
// NewApp assembles the kit application from the europeana domain. The
...
//
// To add a command, declare it in europeana/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := europeana.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(europeana.Domain{}).Register(app)
